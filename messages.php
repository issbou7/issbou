...
$statusValues = array(__("status_all") => "", __("pending") => "Pending", __("queued") => "Queued", __("sent") => "Sent", __("failed") => "Failed", __("received") => "Received", __("scheduled") => "Scheduled");
/** @var User $logged_in_user */
if ($logged_in_user->getReportDelivery()) {
    $statusValues[__("delivered")] = "Delivered";
}
                                            ...
                                            <div class="input-group">
                                                ...
                                                <select class="form-control select2" name="user" id="selectUserInput"
                                                    ...
                                                    foreach ($users as $user) {
                                                        ...
                                                        echo ">{$user->getName()} ({$user->getEmail()})</option>";
                                                    }
                                                    ?>
                                                </select>
                                            </div>
                    ...
                    <form id="reply-message">
                        <div class="modal-body">
                            <input type="hidden" name="mobileNumber" id="numberReplyInput" class="form-control"
                                   required="required">
                            <input type="hidden" name="device" id="deviceReplyInput" class="form-control"
                                   required="required">
                            <input type="hidden" name="sim" id="simReplyInput" class="form-control"
                                   required="required">
                            <div class="form-group">
...
<script>
    $(function () {
        ...
        function getMessages() {
            ...
            ajaxRequest(url, postData).then(result => {
                ...
                $('.reply-message').click(function (event) {
                    ...
                    $('#deviceReplyInput').val($(this).data("device"));
                    $('#simReplyInput').val($(this).data("sim"));
        ...
        $("#resend").click(function (e) {
            ...
            let postData = `messages=${JSON.stringify(getSelectedMessages())}`;
            if (postData) {
